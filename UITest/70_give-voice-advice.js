...
var traverseCells = function(target,collectionView){
	for(var i=1,len=collectionView.cells().length;i<len-1;i+=2){
		...
		target.delay(1);
		target.frontMostApp().mainWindow().collectionViews()[0].cells()[0].buttons()["录音按钮"].touchAndHold(2);
		target.delay(10);
...
UIALogger.logStart("发送语音建议");
target.delay(1);
target.frontMostApp().mainWindow().collectionViews()[0].cells()[0].buttons()["录音按钮"].touchAndHold(4);
target.delay(3);
UIALogger.logPass("发送语音建议");
*/